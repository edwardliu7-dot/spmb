...
function renderDetail(application: ApplicationDetail) {
  ...
  detailElement.innerHTML = `
    <div class="detail-scroll">
      ...
      <section class="detail-section"><h3>Orang tua & wali</h3><dl class="detail-grid">${field("Nomor Kartu Keluarga", application.nomor_kk)}${field("NIK ayah", application.nik_ayah)}${field("Nomor WhatsApp orang tua", application.nomor_hp_orangtua)}${field("Email", application.email)}${field("Nama ayah", application.nama_ayah)}${field("TTL ayah", application.ttl_ayah)}${field("Pendidikan ayah", application.pendidikan_ayah)}${field("Pekerjaan ayah", application.pekerjaan_ayah)}${field("Penghasilan ayah", application.penghasilan_ayah)}${field("Instansi / jabatan ayah", application.instansi_jabatan_ayah)}${field("NIK ibu", application.nik_ibu)}${field("Nama ibu", application.nama_ibu)}${field("TTL ibu", application.ttl_ibu)}${field("Pendidikan ibu", application.pendidikan_ibu)}${field("Pekerjaan ibu", application.pekerjaan_ibu)}${field("Penghasilan ibu", application.penghasilan_ibu)}${field("Instansi / jabatan ibu", application.instansi_jabatan_ibu)}${field("Nama wali", application.nama_wali)}${field("Hubungan wali", application.hubungan_wali)}</dl></section>
  ...
  function renderDetail(application: ApplicationDetail) {
    ...
    document.getElementById("inspector-close")?.addEventListener("click", () => detailElement.classList.remove("is-open"));
     ...
     detailElement.querySelectorAll<HTMLButtonElement>("[data-decision]").forEach((button) => {
       button.addEventListener("click", () => {
         const nextStatus = button.dataset.decision || "";
         if (nextStatus === "Perlu Perbaikan Data") {
           const statusSelect = document.getElementById("application-status") as HTMLSelectElement | null;
           const correctionNote = document.getElementById("correction-note") as HTMLTextAreaElement | null;
           const saveButton = document.getElementById("save-manual-status") as HTMLButtonElement | null;
           const feedback = document.getElementById("status-feedback");
           if (statusSelect) statusSelect.value = nextStatus;
           if (correctionNote) correctionNote.required = true;
           if (saveButton) saveButton.hidden = false;
           if (feedback) {
             feedback.textContent = "Isi catatan perbaikan, lalu klik Simpan status.";
             feedback.className = "decision-feedback";
           }
           correctionNote?.focus();
           return;
         }
         void updateStatus(nextStatus, "Tindakan keputusan tersimpan.");
       });
     });
